...
with tab_ai:
    ...
    if score_row.empty:
        ...
        if st.session_state.get('ai_done'):
            ...
            with m1: st.metric("數據加權項", f"{actual_data_w:.1f}")
            with m2: st.metric("技術加權項", f"{actual_tech_w:.1f}")
